...
import { teams, tournaments } from "../db/repo.js";
import { ratePlayers, suggestTeams } from "../services/balance.js";
import { requireAdminTournament, requireTournamentAccess } from "../services/auth.js";
...
export default async function teamRoutes(app: FastifyInstance) {
  // Participant: current teams in a tournament.
  app.get("/api/tournaments/:tid/teams", { preHandler: requireTournamentAccess }, async (req, reply) => {
  ...
  app.post("/api/tournaments/:tid/teams/generate", { preHandler: requireAdminTournament }, async (req, reply) => {
    const tid = Number((req.params as { tid: string }).tid);
    if (!requireTournament(tid, reply)) return;
  ...
  app.put("/api/tournaments/:tid/teams", { preHandler: requireAdminTournament }, async (req, reply) => {
    const tid = Number((req.params as { tid: string }).tid);
    if (!requireTournament(tid, reply)) return;
  ...
  app.post("/api/tournaments/:tid/teams/lock", { preHandler: requireAdminTournament }, async (req, reply) => {
    const tid = Number((req.params as { tid: string }).tid);
    if (!requireTournament(tid, reply)) return;
  ...
  app.post("/api/tournaments/:tid/teams/unlock", { preHandler: requireAdminTournament }, async (req, reply) => {
    const tid = Number((req.params as { tid: string }).tid);
    if (!requireTournament(tid, reply)) return;
